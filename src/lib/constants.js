...
// a 16x16 grid. These are the standard xterm-256 colors laid out along a
// space-filling Hilbert curve (a locality-preserving 1-D color order poured into
// the grid along a 2-D Hilbert curve), so similar colors cluster both across and
// down: grayscale in the top-left, magentas/reds along the top, blues down the
// right, greens/cyans sweeping the bottom. This is a fixed, hand-verified
// arrangement, so it's spelled out literally below — one source row per grid row
// (see PALETTE_256_ROWS). A handful of colors repeat because xterm-256's system,
// 6x6x6 cube, and grayscale ranges overlap (e.g. #000000, #ffffff, #808080);
// that's harmless — two swatches simply paint the same color. The array order is
// display-only: swatch identity never depends on the index. Each entry is
// { packed, css } to match PENCIL_PALETTE.
//
// WEDGE-SAFETY TRADE-OFF: the previous palette was a sparse 8x8x4 grid whose
// swatches were all >=36/channel apart, so the low-poly wedge gate (wedge-mesh.js
// sameMat, ~12/channel) could never fuse two DISTINCT swatches — the artist had
// exact control over which corners smooth vs. stay crisp. xterm-256 is denser:
// its grayscale ramp steps ~10/channel (within the tolerance), so a staircase of
// ADJACENT gray shades can now auto-smooth into a wedge. The 6x6x6 color-cube
// levels {0,95,135,175,215,255} stay >=40 apart and remain wedge-safe; only
// near-neutral neighbors are affected.
// ---------------------------------------------------------------------------
// prettier-ignore — one row per grid row keeps the source mirroring the layout.
const PALETTE_256_ROWS = [
  '000000 000000 767676 808080 808080 949494 9e9e9e a8a8a8 af5f87 af5faf ff00ff d7005f d70087 d700af ff5f87 ff87af',
  ...
  'ffffaf ffff00 ffff00 ffaf87 ffffff 5f0000 d70000 ff0000 d787ff d75fff ff87d7 ff87ff 8700af 5f00af 5f0087 0000ff',
  'ffffd7 afaf87 ff8787 d7afaf d7875f d75f5f ff0000 875f5f af5fff af87ff 875fd7 875faf 8700d7 8700ff 0000af 0000d7',
  ...
  'af8700 87875f ffd7af ffd787 00ff00 00ff00 5fd787 87d787 5f8787 008787 00875f 008080 5f87ff 5f87d7 87afd7 8787af',
  'afaf00 d78700 afd75f 5fd700 5fff00 87d700 d7ffd7 afd7af 87afaf 5fafaf 5fd7d7 00ffff 00ffaf 00ffd7 afd7ff d7d7ff',
  '808000 ff8700 87d75f afd787 87ff5f 87ff00 afffd7 afffaf 00af87 5fafd7 5fd7af 87d7d7 5fd7ff 00ffff afafff 87afff',
