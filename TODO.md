...
> - **Tier 5 — greedy low-poly base faces:** *not* applied. A regression test
...
- [ ] **🟠 M · Decide the fate of the `vectorize → lowpoly → diagonalize → diag-preview` chain, then act as a unit.**
  ...
  *(Note: `src/lib/vectorize.js` also has a truly-unused export `signedArea` at `:135`
  — resolves automatically on delete.)*
  *(Note: `src/main.js:66` comment "planar remesh: staircases → flat angled facets"
  describes this reverted approach — fix or drop it; see Tier 3.)*
...
- [ ] **🟠 S · README top-view "front direction" prose contradicts its own table and the code.**
  Table `README.md:42` says TOP front → *top* edge; prose `README.md:123` says *bottom*
  row. Code agrees with the table: `src/lib/views.js:117` maps `+z` to image row `v=0`
  (top). The inline comment `src/lib/views.js:108` is wrong the *same* way as the prose.
...
- [ ] **🟠 S · Unify the packed-color representation.**
  ...
  already does. *(Currently harmless — the divergent layout is dev-only — but it's a trap.)*
  → One layout everywhere; replace inline splits with `unpackRGBA`. *(Moot if Tier 2 deletes diagonalize.)*
...
- [ ] **⚪ S · `sliceAtlas` reimplements `deriveTileSize` inline.**
  ...
  can drift and is effectively untested against real slicing). *(Keep the `Math.round`
  at `:70-71` when delegating.)*
