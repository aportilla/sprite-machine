...
//   grid over Set Desktop Pattern. It is an APPLICATION's window — its
//   own: Desktop Patterns is the fourth application (apps/desktop-patterns,
//   the user's call Sep 11 2026 — a desk accessory's seat, its bar File /
//   View), and the panel is adopted as that application's (windows.addPanel's
//   `app`), so its holding the desktop's active state makes Desktop
//   Patterns the front application (shell/windows.js's activation wire) —
//   the Sprite Editor deactivates: the windoids hide, the options strip
//   goes, the bar swaps to the control panel's menus — which also clears
//   the desktop for previewing. Closing it — the close box, its File →
//   Close or Quit — hands active to the topmost document window (the kit
//   promotes the survivor), so the Sprite Editor returns where it was, or
//   leaves the bare desktop to the Finder; a second pick while it's open
//   just brings it forward. Existence IS visibility: the close box removes
//   the window (the document windows' discipline), and nothing about it
//   persists but the pattern itself. Placed by shell/layout.js centeredBox
//   and adopted by windows.js as a PANEL, so Arrange Windows re-centers it
...
export function initPatterns(desktop, windows, { saved = null } = {}) {
  ...
  function open() {
    ...
    // and this one's owner is the Desktop Patterns application (see the
    // header), whose menus the bar shows while it is front.
    desktop.append(win);
    const size = { width: win.width ?? 0, height: win.height ?? 0 };
    windows.addPanel(win, (w, h) => centeredBox(w, h, size), null, {
  ...
  return {
     ...
     *  (apps/desktop-patterns); also the HMR teardown's. */
    close,
