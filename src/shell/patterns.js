...
//   its holding the desktop's active state reads as "no document window is
//   active" (shell/windows.js's activation wire) — the application
//   deactivates: the windoids hide, the options strip goes, the
//   document-scoped menus grey — which also clears the desktop for
//   previewing. Closing it hands active to the topmost document window (the
//   kit promotes the survivor), so the application returns where it was; a
...
export function initPatterns(desktop, windows, { saved = null } = {}) {
  ...
  function open() {
    ...
    windows.addPanel(win, (w, h) => centeredBox(w, h, size));
